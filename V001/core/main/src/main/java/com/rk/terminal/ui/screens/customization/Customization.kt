...
private fun showLog(title: String, message: String) {
    MainActivity.showLog("Customization", "[$title] $message")
}

// Oder kürzer:
// private fun showLog(message: String) = MainActivity.showLog("Customization", message)
...
fun Customization(modifier: Modifier = Modifier) {
    ...
    // ★ Log: Customization geöffnet
    showLog("Info", "🎨 Customization Screen geöffnet")
    showLog("Debug", "🎨 Font: ${Settings.custom_font_name}, Background: ${Settings.custom_background_name}")
    ...
    PreferenceLayout(label = stringResource(strings.customizations)) {
            ...
            PreferenceTemplate(title = {}) {
                Slider(
                    ...
                    onValueChange = {
                        ...
                        showLog("Debug", "🔤 Schriftgröße geändert: ${it.toInt()}dp")
                    },
        ...
        PreferenceGroup {
            ...
            val fontLauncher = rememberLauncherForActivityResult(
                ...
                uri?.let {
                    showLog("Info", "🔤 Font ausgewählt: $uri")
                    scope.launch(Dispatchers.IO){
                        ...
                        showLog("Info", "✅ Font geladen: $name")
                    }
                }
            }
            ...
            PreferenceTemplate(
                modifier = Modifier.clickable(onClick = {
                    showLog("Debug", "🔤 Font-Auswahl gestartet")
                    scope.launch{
                ...
                endWidget = {
                    if (fontExists){
                        IconButton(onClick = {
                            showLog("Info", "🗑️ Font gelöscht: $fontName")
                            scope.launch{
                                font.delete()
        ...
        PreferenceGroup {
            ...
            val launcher = rememberLauncherForActivityResult(
                ...
                uri?.let {
                    showLog("Info", "🖼️ Hintergrund ausgewählt: $uri")
                    scope.launch(Dispatchers.IO){
                        ...
                        backgroundName = name
                        showLog("Debug", "🖼️ Hintergrund gespeichert: $name")
                        ...
                        withContext(Dispatchers.IO) {
                            ...
                            bitmap.value?.apply {
                                ...
                                Palette.from(androidBitmap).generate { palette ->
                                    ...
                                    showLog("Debug", "🎨 Textfarbe: ${if (blackText) "schwarz" else "weiß"}")
                                }
                            }
                        }
                        imageExists = image.exists()
                        showLog("Info", "✅ Hintergrund geladen: $name")
                    }
                }
            }
            ...
            PreferenceTemplate(
                modifier = Modifier.clickable(onClick = {
                    showLog("Debug", "🖼️ Hintergrund-Auswahl gestartet")
                    scope.launch{
                ...
                endWidget = {
                    ...
                    if (imageExists){
                        IconButton(onClick = {
                            showLog("Info", "🗑️ Hintergrund gelöscht: $backgroundName")
                            scope.launch{
                                image.delete()
        ...
        PreferenceGroup {
            ...
            PreferenceTemplate(title = {}){
                Slider(
                    ...
                    onValueChange = {
                        wallAlpha = it
                        showLog("Debug", "🎨 Wallpaper Alpha: $it")
                    },
                    onValueChangeFinished = {
                        Settings.wallTransparency = wallAlpha
                        showLog("Debug", "💾 Wallpaper Alpha gespeichert: $wallAlpha")
                    }
                )
            }
        }
        ...
        PreferenceGroup {
            SettingsToggle(label = stringResource(strings.bell), description = stringResource(strings.bell_desc), showSwitch = true, default = Settings.bell, sideEffect = {
                Settings.bell = it
                showLog("Info", "🔔 Bell: ${if (it) "aktiviert" else "deaktiviert"}")
            })
            ...
            SettingsToggle(label = stringResource(strings.vibrate), description = stringResource(strings.vibrate_desc), showSwitch = true, default = Settings.vibrate, sideEffect = {
                Settings.vibrate = it
                showLog("Info", "📳 Vibrate: ${if (it) "aktiviert" else "deaktiviert"}")
            })
        }
        ...
        PreferenceGroup {
            SettingsToggle(
                ...
                default = Settings.statusBar, sideEffect = {
                    ...
                    showLog("Info", "📊 StatusBar: ${if (it) "aktiviert" else "deaktiviert"}")
                })
            ...
            SettingsToggle(
                ...
                default = Settings.horizontal_statusBar, sideEffect = {
                    ...
                    showLog("Info", "📊 Horizontal StatusBar: ${if (it) "aktiviert" else "deaktiviert"}")
                })
            ...
            val sideEffect:(Boolean)-> Unit = {
                if (!it && showToolbar.value){
                    showLog("Warn", "⚠️ Toolbar-Warnung angezeigt (User versucht zu deaktivieren)")
                    MaterialAlertDialogBuilder(context).apply {
                        ...
                        setPositiveButton("OK"){_,_ ->
                            ...
                            showLog("Info", "🔧 Toolbar: ${if (it) "aktiviert" else "deaktiviert"} (nach Warnung)")
                        }
                    ...
                    Settings.toolbar = it
                    showToolbar.value = it
                    showLog("Info", "🔧 Toolbar: ${if (it) "aktiviert" else "deaktiviert"}")
                }
            }
            ...
            SettingsToggle(
                ...
                default = Settings.toolbar_in_horizontal, sideEffect = {
                    ...
                    showLog("Info", "🔧 Horizontal Toolbar: ${if (it) "aktiviert" else "deaktiviert"}")
                })
            SettingsToggle(
                ...
                default = Settings.virtualKeys, sideEffect = {
                    ...
                    showLog("Info", "⌨️ Virtual Keys: ${if (it) "aktiviert" else "deaktiviert"}")
                })
            ...
            SettingsToggle(
                ...
                default = Settings.hide_soft_keyboard_if_hwd, sideEffect = {
                    ...
                    showLog("Info", "⌨️ Hide Soft Keyboard: ${if (it) "aktiviert" else "deaktiviert"}")
                })
        }
        ...
        PreferenceGroup(heading = stringResource(strings.keyboard_shortcuts)) {
            ...
            SettingsToggle(
                ...
                sideEffect = {
                    ...
                    shortcutsEnabled = it
                    showLog("Info", "⌨️ Keyboard Shortcuts: ${if (it) "aktiviert" else "deaktiviert"}")
                })
            ...
            for (action in ShortcutAction.entries) {
                ...
                SettingsToggle(
                    ...
                    default = false,
                    sideEffect = { 
                        showLog("Debug", "⌨️ Shortcut Capture gestartet für: ${action.name}")
                        showCaptureFor = action 
                    },
                )
            }
            ...
            if (showCaptureFor != null) {
                ShortcutCaptureDialog(
                    ...
                    onDismiss = { 
                        showLog("Debug", "⌨️ Shortcut Capture abgebrochen für: ${showCaptureFor?.name}")
                        showCaptureFor = null 
                    },
                    onConfirm = { binding ->
                        showLog("Info", "⌨️ Shortcut gespeichert: ${showCaptureFor?.name} -> ${binding.toDisplayString()}")
