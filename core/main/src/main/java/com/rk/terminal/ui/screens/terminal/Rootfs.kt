...
object Rootfs {
    ...
    // Oder kürzer:
    // private fun showLog(message: String) = MainActivity.showLog("Rootfs", message)
    ...
    fun isFilesDownloaded(): Boolean {
        ...
        if (exists) {
            showLog("Debug", "✅ Alle Dateien vorhanden")
        } else {
            ...
            if (!reTerminal.child("alpine.tar.gz").exists()) missingFiles.add("alpine.tar.gz")
    ...
    /**
     * Zeigt alle Dateien im reTerminal-Verzeichnis an
     */
    ...
    /**
     * Zeigt detaillierte Informationen über den Rootfs-Status
     */
    fun showStatus() {
