...
private fun showLog(title: String, message: String) {
    MainActivity.showLog("ShortcutCaptureDialog", "[$title] $message")
}

// Oder kürzer:
// private fun showLog(message: String) = MainActivity.showLog("ShortcutCaptureDialog", message)
...
fun ShortcutCaptureDialog(
    ...
    // ★ Log: Dialog geöffnet
    showLog("Info", "🔓 ShortcutCaptureDialog geöffnet für Aktion: ${action.name}")

    Dialog(onDismissRequest = {
        showLog("Debug", "🔓 Dialog dismissed")
        onDismiss()
    }) {
        Surface(
            ...
            modifier = Modifier
                ...
                .onPreviewKeyEvent { event ->
                    ...
                    // ★ Log: Tastendruck
                    showLog("Debug", "⌨️ KeyEvent: keyCode=$keyCode, key=${android.view.KeyEvent.keyCodeToString(keyCode)}")
                    ...
                    if (ShortcutBinding.isModifierKey(keyCode)) {
                        ...
                        // ★ Log: Modifier-Taste
                        if (parts.isNotEmpty()) {
                            showLog("Debug", "⌨️ Modifier: ${parts.joinToString(" + ")}")
                        }
                    ...
                    if (ShortcutBinding.isReservedKey(keyCode)) {
                        ...
                        // ★ Log: Reservierte Taste
                        showLog("Warn", "⚠️ Reservierte Taste: ${android.view.KeyEvent.keyCodeToString(keyCode)}")
                        return@onPreviewKeyEvent true
                    }

                    // Non-modifier key pressed — finalize capture
                    val binding = ShortcutBinding.fromKeyEvent(native)
                    showLog("Debug", "⌨️ Binding erfasst: ${binding.toDisplayString()}")
                    ...
                    if (conflict != null) {
                        ...
                        captured = binding
                        // ★ Log: Konflikt
                        showLog("Warn", "⚠️ Konflikt mit Aktion: ${conflict.name}")
                    } else {
                        conflictMessage = null
                        captured = binding
                        // ★ Log: Neues Shortcut erfasst
                        showLog("Info", "✅ Shortcut erfasst: ${binding.toDisplayString()} für ${action.name}")
                    }
                    modifierHint = ""
                    true
                }
            ...
            Column(
                ...
                Row(
                    ...
                    TextButton(onClick = {
                        showLog("Info", "🧹 Shortcut gelöscht für ${action.name}")
                    ...
                    Row {
                        TextButton(onClick = {
                            showLog("Debug", "❌ Dialog abgebrochen für ${action.name}")
                            onDismiss()
                        }) {
                        ...
                        TextButton(
                            onClick = {
                                captured?.let { 
                                    showLog("Info", "✅ Shortcut bestätigt: ${it.toDisplayString()} für ${action.name}")
                                    onConfirm(it) 
                                }
                            },
        ...
        LaunchedEffect(windowInfo.isWindowFocused) {
            if (windowInfo.isWindowFocused) {
                showLog("Debug", "🎯 Fenster fokussiert, FocusRequest gestartet")
