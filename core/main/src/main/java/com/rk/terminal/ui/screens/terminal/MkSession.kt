...
object MkSession {
    ...
    // ============================================================
    @JvmStatic  // ← DAS IST DIE ÄNDERUNG!
    fun createSession(
        ...
        showLog("Info", "🔍 Erstelle Session: $session_id, workingMode: $workingMode")
        ...
        with(activity) {
            ...
            val shell = if (pendingCommand == null) {
                args = if (workingMode == WorkingMode.ALPINE) {
                    showLog("Info", "🐧 Starte Alpine mit init-host")
                    arrayOf("-c", initFile.absolutePath)
                } else {
                    showLog("Info", "📟 Starte lokale Shell")
                    arrayOf()
                }
                "/system/bin/sh"
            } else {
