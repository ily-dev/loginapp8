...
class TargetAndroid(Target):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if config.getdefault('app', 'p4a.bootstrap', 'sdl2') != 'qt':
            # add qt-libs
            self.extra_p4a_args.append("--qt-libs")
            
            # add load-local-libs
            self.extra_p4a_args.append("--load-local-libs")
    ...
    def execute_build_package(self, build_cmd):
        ...
        # --- UNSER PATCH: PROJEKTPFAD IN LOCAL.PROPERTIES SCHREIBEN ---
        try:
            import os
            # 1. Den echten Projektordner ermitteln (wo die buildozer.spec liegt)
            #echter_projekt_ordner = os.path.dirname(os.path.abspath(self.buildozer.spec_filename))
            #self.buildozer.info(str(echter_projekt_ordner))
            
            # 2. Den dists-Ordner ermitteln
            #dists_ordner = self.get_dist_dir()
            
            # 3. Pfad zur local.properties im dists-Ordner festlegen
            #self.buildozer.info(str(dists_ordner))
            #prop_datei = os.path.join(dists_ordner, "local.properties")
            
            # 4. Falls der dists-Ordner noch gar nicht existiert, erstellen wir ihn kurz
            #if not os.path.exists(dists_ordner):
                #os.makedirs(dists_ordner, exist_ok=True)
                
            # 5. Datei einlesen, falls sie schon existiert
            #zeilen = []
            #if os.path.exists(prop_datei):
                #with open(prop_datei, "r", encoding="utf-8") as f:
                    #zeilen = f.readlines()
            
            # Altes 'projekt.quelle' entfernen und neu hinzufügen
            #neue_zeilen = [z for z in zeilen if not z.startswith("projekt.quelle=")]
            #neue_zeilen.append(f"projekt.quelle={echter_projekt_ordner}\n")
            
            # Zurück in die Datei schreiben
            #with open(prop_datei, "w", encoding="utf-8") as f:
                #f.writelines(neue_zeilen)
                
            #self.buildozer.info(f"[BUILDOZER PATCH] Projektpfad erfolgreich hinterlegt: {echter_projekt_ordner}")
        except Exception as e:
            self.buildozer.error(f"[BUILDOZER PATCH FEHLER] Konnte Pfad nicht schreiben: {e}")
        # --- ENDE DES PATCHES ---
        ...
        # Qt-spezifische Argumente (nur wenn Bootstrap = qt)
        if self._p4a_bootstrap == 'qt':
            # Qt-Bibliotheken (die für Android gebaut wurden)
            qt_libs = "Qt5Core,Qt5Gui,Qt5Widgets,Qt5Qml,Qt5Quick"
            cmd.append("--qt-libs={}".format(qt_libs))
            
            # Lokale Qt-Bibliotheken (Pfade zu den .so-Dateien)
            # Diese müssen im NDK-Verzeichnis liegen
            load_local_libs = "libQt5Core.so:libQt5Gui.so:libQt5Widgets.so"
            cmd.append("--load-local-libs={}".format(load_local_libs))
            
            # Qt-Initialisierungsklassen (für JNI)
            init_classes = "org.qtproject.qt.android.QtNative,org.qtproject.qt.android.QtActivity"
            cmd.append("--init-classes={}".format(init_classes))
    ...
    def _generate_whitelist(self, dist_dir):
        ...
        self.buildozer.debug('project.whitelist updated')
    ...
    def build_package(self):
        ...
        if config.getdefault('app', 'p4a.bootstrap', 'qt') == 'qt':
            # add qt-libs
            build_cmd += [("--qt-libs", "Qt5Core,Qt5Gui,Qt5Widgets")]
            
            # add load-local-libs
            build_cmd += [("--load-local-libs", "libQt5Core.so,libQt5Gui.so" )]
        ...
        if is_gradle_build:
            ...
            artifact = u'app-{mode}.{artifact_format}'.format(
                packagename=packagename_src, mode=mode, artifact_format=self.artifact_format)
            if self.artifact_format == "apk":
                artifact_dir = join(dist_dir, "app" ,"build", "outputs", "apk", mode_sign)
            elif self.artifact_format == "aab":
                artifact_dir = join(dist_dir, "app" ,"build", "outputs", "bundle", mode_sign)
            elif self.artifact_format == "aar":
                artifact_dir = join(dist_dir, "app", "build", "outputs", "aar")
