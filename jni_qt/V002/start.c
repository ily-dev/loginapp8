...
int main(int argc, char *argv[]) {
    ...
    // ============================================================
            ...
            wchar_t *wchar_zip = Py_DecodeLocale(python_path, NULL);
            if (wchar_zip != NULL) {
                PyWideStringList_Append(&config.module_search_paths, wchar_zip);
                LOGP("✅ config.module_search_paths = %s", python_path);
                LOG("start.c", "✅ config.module_search_paths gesetzt");
            }

            LOGP("🐍 PY_MAJOR_VERSION = %d", PY_MAJOR_VERSION);
            LOGP("🐍 PY_MINOR_VERSION = %d", PY_MINOR_VERSION);
            LOGP("🐍 Python 3.11+ Modus: Py_InitializeFromConfig()");
            ...
            if (PyStatus_Exception(status)) {
                ...
                // Fallback: Versuche Py_Initialize()
                Py_Initialize();
