...
estimate_backup_size() {
    ...
    # 🔥 SCHNELL: Ein einziger du-Aufruf für das ganze Verzeichnis
    local total_size=$(du -sb --exclude='tmp' --exclude='proc' --exclude='sys' \
        --exclude='dev' --exclude='run' --exclude='vendor' --exclude='system' \
        --exclude='apex' --exclude='sdcard' --exclude='storage' --exclude='mnt' \
        --exclude='boot' --exclude='media' . 2>/dev/null | awk '{print $1}')
    
    # Fallback, falls --exclude nicht unterstützt wird
