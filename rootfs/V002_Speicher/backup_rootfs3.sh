...
analyze_storage() {
    ...
    # Wichtige Verzeichnisse analysieren
    for dir in bin etc lib root sbin usr var home opt srv; do
        if [ -d "$dir" ]; then
            # Größe ermitteln (ohne ausgeschlossene Unterordner)
    ...
    # Zusätzlich: tmp, var/cache, var/log (falls vorhanden)
    for dir in tmp var/cache var/log var/tmp root/.cache; do
        if [ -d "$dir" ]; then
            local size=$(du -sb "$dir" 2>/dev/null | awk '{print $1}')
            if [ -n "$size" ] && [ "$size" -gt 0 ]; then
                printf "%-20s %15s\n" "/$dir" "$(format_size $size)"
...
create_backup() {
    ...
    # Backup mit Fortschrittsanzeige über Pipe
    local progress_file="/data/local/tmp/backup_progress_$$"
