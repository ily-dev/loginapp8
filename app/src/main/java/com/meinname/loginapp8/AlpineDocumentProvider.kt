...
class AlpineDocumentProvider : DocumentsProvider() {
    ...
    private val BASE_DIR: File by lazy { FileUtil.alpineHomeDir(context!!) }

    override fun queryRoots(projection: Array<String>?): Cursor {
        val result = MatrixCursor(
            projection ?: DEFAULT_ROOT_PROJECTION
        )
        // 👇 HIER WIRD DER NAME JETZT DYNAMISCH EINGESETZT:
        val applicationName = "loginapp8"
    ...
    override fun querySearchDocuments(
        ...
        while (!pending.isEmpty() && result.count < MAX_SEARCH_RESULTS) {
            ...
            try {
                isInsideHome = file.canonicalPath.startsWith(BASE_DIR.canonicalPath) 
            } catch (e: IOException) {
                isInsideHome = true
            }
